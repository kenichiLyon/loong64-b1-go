...
- `jobs`：上传文件会创建 `submission_artifact_parse` 队列任务，后续 worker 可继续执行 OCR、代码深层静态分析和更复杂文档抽取。
...
- DOCX：按 ZIP 容器做安全检查，并从 `word/document.xml` 抽取正文摘要、段落数和表格数。
- PDF：使用纯 Go 解析器提取页数和基础文本摘要。
- DOC：登记元数据和对象存储 key，旧式二进制 Word 深度抽取仍保留给后续 worker 或人工复核。
...
- OCR 与旧式 DOC 深度解析仍属于高风险能力，后续引入前必须验证 linux/loong64 可用性，并保留人工查看原始文件入口。
