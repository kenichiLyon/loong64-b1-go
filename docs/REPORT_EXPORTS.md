...
Stage 6 MVP 提供个人评价报告、实验统计摘要、HTML/CSV 导出和 PDF 降级记录。实现原则是 LoongArch + 银河麒麟优先：默认不引入浏览器、LibreOffice headless、wkhtmltopdf 或 CGO/native 依赖。
...
- 教师/管理员：可查看有课程授权的提交报告和实验统计，可创建自己的导出任务。
...
- 文件保存到 `STORAGE_ROOT/reports/{scope_type}/{scope_id}/{export_id}.{html|csv}`。
