...
本文从代码结构、运行拓扑、核心模块、数据模型和主业务链路几个层面说明当前项目架构。更细的专项说明仍以各专题文档为准，例如部署见 `docs/DEPLOY_KYLIN.md`，Python 推理微服务见 `docs/PYTHON_AI_MIDDLEWARE.md`，安全见 `docs/SECURITY.md`。
...
- `Python 推理微服务`负责附件解析、检索上下文、RAG 组装和模型推理相关能力。
- `Web 前端`是 PC 工作台，覆盖管理员、教师、学生与部署初始化流程。
- `数据库 + 本地对象存储`保存业务状态、审计记录、附件和报表导出文件。

一句话边界：

```text
Go 管业务，Python 管推理，Web 管交互，DB/Storage 管状态与文件。
```
...
- Go 主服务可不启用 Python 服务，但当前正式推荐路径是双服务协作。
- 当 Go 配置了 `AI_GATEWAY_BASE_URL` 时，Go 会把 Python 服务纳入 readiness 检查，并优先使用 Python 进行附件解析和 AI 初评。
- Go 侧也保留 `LLM_BASE_URL` 直连 OpenAI-compatible 模型的能力，可用于初评兜底和部署助手。
- Python 服务自身使用 `AI_GATEWAY_LLM_*` 环境变量连接模型服务。
...
internal/migrate/        方言感知迁移执行器
internal/storage/        本地对象存储封装
internal/aigateway/      Go 调 Python AI Gateway 的客户端
...
deploy/kylin/            银河麒麟 systemd、env、nginx 和验收脚本
...
- `cmd/migrate/main.go`：独立迁移命令，按 `DB_DRIVER` 选择 PostgreSQL 或 SQLite 迁移目录。
...
- 数据库：`DB_DRIVER=sqlite|postgres`、`SQLITE_PATH`、`DATABASE_URL`、`AUTO_MIGRATE`。
- 存储：`STORAGE_ROOT`，默认 `./storage`。
- 会话：`SESSION_COOKIE_NAME`、`CSRF_COOKIE_NAME`、`SESSION_TTL`、`SESSION_SECURE_COOKIE`。
- Python 微服务：`AI_GATEWAY_BASE_URL`、`AI_GATEWAY_API_KEY`、`AI_GATEWAY_TIMEOUT`。
...
`internal/migrate.Runner` 根据数据库驱动选择迁移目录：

- PostgreSQL：`migrations/*.sql`
- SQLite：`migrations/sqlite/*.sql`

迁移通过 `schema_migrations` 记录版本和 checksum，已应用迁移内容变化会报错。
...
发布态可先执行 `npm run build --prefix web`，再用 `go build -tags webui ./cmd/server` 构建内嵌前端的完整二进制。

## 6. Python AI Gateway

Python 服务位于 `python-ai-gateway/`，使用 FastAPI。
...
- 新增模型调用能力：Go 编排放 `internal/llm` 或 `internal/aigateway`；复杂解析/RAG/模型适配优先放 Python 微服务。
...
- `docs/PYTHON_AI_MIDDLEWARE.md`：Python 推理微服务。
