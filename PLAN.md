...
首个 Stage 6 MVP 切片已确定为“纯 Go、LoongArch 安全优先”的报表闭环：先交付个人报告、实验统计、HTML/CSV 导出记录与下载，PDF 暂按降级策略记录为待配置，避免在未验证的 LoongArch + 银河麒麟环境中引入浏览器、LibreOffice headless 或 native PDF 依赖。
...
- PC Web 报表面板：支持个人报告预览、实验统计预览、HTML/CSV 导出和 PDF 降级结果展示。

后续交付：

- 班级/课程跨实验统计面板，包含提交完成率、跨班级对比和模型初评与教师改分差异。
...
- 个人报告和实验统计 API 可在教师/学生权限下正确返回或拒绝访问。
