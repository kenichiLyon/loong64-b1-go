...
if [ ! -f "$APP_CONFIG/loong64-b1-go.env" ]; then
  ...
  echo "Created $APP_CONFIG/loong64-b1-go.env; edit database and AI Worker settings before starting."
fi
...
if [ ! -f "$APP_CONFIG/python-ai-gateway.env" ]; then
  ...
  echo "Created $APP_CONFIG/python-ai-gateway.env; copy the current Python AI Worker implementation into $APP_HOME/python-ai-gateway, then edit model gateway settings before starting."
fi

install -o root -g root -m 0644 deploy/kylin/systemd/loong64-b1-go.service /etc/systemd/system/loong64-b1-go.service
install -o root -g root -m 0644 deploy/kylin/systemd/loong64-b1-upgrade.service /etc/systemd/system/loong64-b1-upgrade.service
...
echo "Copy the Go release bundle into $APP_HOME/bin."
echo "Copy the current Python AI Worker implementation into $APP_HOME/python-ai-gateway."
echo "Then run:"
echo "  systemctl enable --now python-ai-gateway.service"
echo "  systemctl start loong64-b1-upgrade.service"
