...
func TestAuthLoginLogoutAndMe(t *testing.T) {
	...
	meReq.AddCookie(cookieByName(cookies, cfg.SessionCookieName))
	...
	logoutReq.AddCookie(cookieByName(cookies, cfg.SessionCookieName))
	...
	meAfterReq.AddCookie(cookieByName(cookies, cfg.SessionCookieName))
...
func TestAdminCanSetUserPasswordAndLogin(t *testing.T) {
	...
	if studentLoginRec.Code != http.StatusOK {
		t.Fatalf("student login code: %d body=%s", studentLoginRec.Body.String())
	}
}
